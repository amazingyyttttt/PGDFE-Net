...
#     parser.add_argument('--config',default='/home/lucid/lwt/code/PETDet/configs/oriented_rcnn/pcloriented_rcnn_r50_fpn_3x_shiprs3_le90.py', help='test config file path')
#     parser.add_argument('--checkpoint',default='/home/lucid/lwt/code/PETDet/pcl_dir/shipsr3/best_mAP_epoch_35.pth', help='checkpoint file')
#     parser.add_argument(
#         '--repeat-num',
#         type=int,
#         default=1,
...
def parse_args():
    parser = argparse.ArgumentParser(description='mmrotate benchmark a model')
    parser.add_argument('--config', default='/home/lucid/lwt/code/PETDet/xiaorong/gl_hrsc/hree/pet_pcl_cc.py',
                        help='test config file path')
    parser.add_argument('--checkpoint', default='/home/lucid/lwt/code/PETDet/xiaorong/gl_hrsc/hree/best_mAP_epoch_33.pth',
...
def measure_inference_speed(cfg, checkpoint, max_iter, log_interval,
                            is_fuse_conv_bn, use_fp16, distributed):
    """Inference speed statistics."""
    if cfg.get('cudnn_benchmark', False):
        torch.backends.cudnn.benchmark = True
    cfg.model.pretrained = None
    ...
    for i, data in enumerate(data_loader):
        ...
        if i >= num_warmup:
            ...
            if (i + 1) % log_interval == 0:
                fps = (i + 1 - num_warmup) / pure_inf_time
                print(f'Done image [{i + 1:<3}/ {max_iter}], '
                      f'fps: {fps:.1f} img / s, '
                      f'times per image: {1000 / fps:.1f} ms / img', flush=True)
...
def repeat_measure_inference_speed(cfg, checkpoint, max_iter,
                                   log_interval, is_fuse_conv_bn,
                                   use_fp16, repeat_num, distributed):
    assert repeat_num >= 1
    fps_list = []
    for _ in range(repeat_num):
        cp_cfg = copy.deepcopy(cfg)
        fps_list.append(
            measure_inference_speed(
                cp_cfg, checkpoint, max_iter, log_interval,
                is_fuse_conv_bn, use_fp16, distributed))

    if repeat_num > 1:
        fps_round = [round(f, 1) for f in fps_list]
        times_round = [round(1000 / f, 1) for f in fps_list]
        mean_fps = sum(fps_round) / len(fps_round)
        mean_time = sum(times_round) / len(times_round)
        print(f'Overall fps: {fps_round}[{mean_fps:.1f}] img / s, '
              f'times per image: {times_round}[{mean_time:.1f}] ms / img', flush=True)
        return fps_list
    return fps_list[0]
...
def main():
    ...
    if args.launcher != 'none':
        ...
        print('Warning: running in single-GPU mode without distributed.')

    repeat_measure_inference_speed(
        cfg, args.checkpoint, args.max_iter, args.log_interval,
        args.fuse_conv_bn, args.use_fp16, args.repeat_num, distributed)

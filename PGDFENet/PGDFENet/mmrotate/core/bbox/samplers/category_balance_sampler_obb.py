...
class OBBCateBalanceSampler(RRandomSampler):
    ...
    def random_category_balance_choice(self, assign_result,gallery, num):
        ...
        categorys_num = len(categorys_id)  # 确保 categorys_num 和 categorys_id 长度一致
        ...
        ########################################################
        # unrand_inds = torch.tensor([]).to(gallery.device)
        #######################################################
        rand_inds = rand_inds.flatten() #rand_inds = rand_inds.flatten()

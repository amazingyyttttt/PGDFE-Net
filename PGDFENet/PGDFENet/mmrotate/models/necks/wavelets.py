...
class DWT_2D(nn.Module):
    ...
    def forward(self, x):
        dim = x.shape[1]  # 通道C
        x_ll = torch.nn.functional.conv2d(x, self.w_ll.expand(dim, -1, -1, -1), stride=2,
                                          groups=dim)  # torch.nn.functional.conv2d(input,filters,bias,stride,padding,dilation,groups)
        x_lh = torch.nn.functional.conv2d(x, self.w_lh.expand(dim, -1, -1, -1), stride=2,
                                          groups=dim)  # filters代表卷积核的大小(out_channels，in_channe/groups，H，W)，是一个四维tensor
...
#
#     wave = IDWT_2D(wave='rbio1.1')
#
#     output_tensor = wave(input_tensor)
#

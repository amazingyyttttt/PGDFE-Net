...
from .attention import Refine
from .patch_ge import Patch_Attention, Patch_AttentionV2, Attention_Embedding
from .msm import MSM
...
class WFR(nn.Module):
    ...
    def __init__(self,
        ...
        self.aem = Attention_Embedding(in_channels=in_channels, out_channels=in_channels)
    ...
    def forward(self, inputs):
        ...
        for i in range(2):
            shuotcut = feats[i]
            feats[i] = self.aem(feats[i], feats[i+2])
            # feats[3] = self.conv1(feats[3])
            # feats[4] = self.conv1(feats[4])
            feats[i] = feats[i] + shuotcut

...
class LeaSupConProxyAnchorLoss(torch.nn.Module):
    def __init__(self, class_num=37, size_contrast=512, stage = 1, mrg=0, alpha=32, loss_weight=0.5):
        ...
        self.stage = stage
        self.proxy_per_cls = 1
        self.proxies = torch.nn.Parameter(torch.randn((self.nb_classes+1)*self.proxy_per_cls, self.sz_embed).cuda())     # [51, 512]可训练，初始化为正态分布随机数
    ...
    def init_proxies(self,features,labels,cls_scores):
        ...
        if self.iter_now < 10001:
            ...
            self.iter_now = 0
            torch.save(self.proxies, '/home/lucid/lwt/code/PETDet/work_dir/proxy_anchor_init/'+str(self.epoch_now)+'.pt')
    ...
    #     cos = F.linear(X, P)  # Calcluate cosine similarity     cos = x * P转置
    #     # 制作onehot标签
    ...
    #     with_pos_proxies = torch.nonzero(P_one_hot.sum(dim=0) != 0).squeeze(dim=1)  # The set of positive proxies of data in the batch    有正样本的类
...
##########################################################################################################
    def forward(self, feature, label):
        sim_matrix = torch.mm(F.normalize(feature, dim=-1), F.normalize(self.proxies, dim=-1).t().contiguous())
        mask = torch.zeros_like(sim_matrix)

        for j in range(self.proxy_per_cls):
            mask[self.proxy_per_cls * label + j] = 1
        # for i in range(2):
        #     for j in range(self.proxy_per_cls):
        #         mask[i][self.proxy_per_cls * label[i] + j] = 1
        unsim = sim_matrix.masked_select((torch.ones_like(mask) - mask).bool()).mean()
        sim = - sim_matrix.masked_select(mask.bool()).mean()


        prototype_sim_matrix = torch.mm(F.normalize(self.proxies, dim=-1), F.normalize(self.proxies, dim=-1).t().contiguous())
        # b = torch.ones_like(prototype_sim_matrix)
        # a = torch.eye(self.proxy_per_cls * (self.nb_classes + 1), device=prototype_sim_matrix.device)
        prototype_mask = (torch.ones_like(prototype_sim_matrix) - torch.eye(self.proxy_per_cls * (self.nb_classes + 1),
                                                                            device=prototype_sim_matrix.device)).bool()
        prototypeLoss = prototype_sim_matrix.masked_select(prototype_mask).mean()
        a = 1.0
        b = 1.0
        loss = a * unsim + b * sim + prototypeLoss
        return loss